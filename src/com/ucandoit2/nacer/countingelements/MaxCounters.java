package com.ucandoit2.nacer.countingelements;
        ...
        */
        ...
        for (int i = 0; i < A.length; i++) {
            if (A[i] >= 1 && A[i] <= N) { // normal case
                ...
                if (myArray[A[i] - 1] < min) {
                    myArray[A[i] - 1] = min; // update it to "min"
                }

                myArray[A[i] - 1]++;  // increased by 1

                if (myArray[A[i] - 1] > max) { // maintain max
                    max = myArray[A[i] - 1];
                }
