 ...
 * area is flooded. You can assume that the altitude outside this area is zero and the outside area can accommodate
 ...
 * water filling the low-lying areas with maximum possible volume. Thus, blocks 3 and 5 have a water depth of 2 while
...
public class FloodDepth {
    public static void main(String[] args) {
        System.out.println("FloodDepth:\n\t" + new FloodDepth().solution(new int[]{1, 3, 2, 1, 2, 1, 5, 3, 3, 4, 2}));
    }
    ...
    public int solution(int[] A) {
        ...
        for (int current : A) {
            if (current > firstMax) {
                answer = getAnswer(firstMax, current, minBetween, answer);
                firstMax = current;
                minBetween = current;
            }
            if (current > prev)
    ...
    private int getAnswer(int first, int second, int minBetween, int answer) {
        int tempAnswer = min(first, second) - minBetween;
        return tempAnswer > answer ? tempAnswer : answer;
    }
}
